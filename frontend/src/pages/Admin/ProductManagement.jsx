...
const ProductManagement = () => {
  ...
  const [newProductBrandId, setNewProductBrandId] = useState(''); // أصبح اختياريًا
  ...
  const [newProductAdditionalImageFiles, setNewProductAdditionalImageFiles] = useState([]); // لملفات الصور الإضافية الجديدة
  const [newProductAdditionalImagePreviews, setNewProductAdditionalImagePreviews] = useState([]); // لمعاينات الصور الإضافية الجديدة
  const [existingAdditionalImages, setExistingAdditionalImages] = useState([]); // للصور الإضافية الموجودة للمنتج الذي يتم تعديله
  const [removedAdditionalImageIds, setRemovedAdditionalImageIds] = useState([]); // لمعرفات الصور الإضافية المحذوفة
  ...
  // معالجة تغيير ملف الصورة الرئيسية
  ...
  // معالجة إزالة الصورة الرئيسية الحالية
  ...
  // معالجة تغيير ملفات الصور الإضافية
  ...
  // إزالة صورة إضافية جديدة قبل الرفع
  ...
  // إزالة صورة إضافية موجودة (عند التعديل)
  ...
  const handleSubmit = async (e) => {
    ...
    const productData = {
      ...
      brand_id: newProductBrandId === '' ? null : newProductBrandId, // إرسال null إذا كان فارغاً
  ...
  const handleEdit = (product) => {
    ...
    setNewProductBrandId(product.brand_id || ''); // تأكد من التعامل مع null
    setNewProductMainImageFile(null);
    setNewProductMainImagePreview(product.main_image_url || null);
    setNewProductAdditionalImageFiles([]); // مسح الملفات الجديدة
    setNewProductAdditionalImagePreviews([]); // مسح المعاينات الجديدة
    setExistingAdditionalImages(product.images || []); // تحميل الصور الإضافية الموجودة
    setRemovedAdditionalImageIds([]); // مسح أي معرفات حذف سابقة
  ...
  return (
    <div className="container-fluid">
      ...
      {/* نموذج إضافة/تعديل منتج - تصميم جذاب */}
        ...
        <div className="card-body p-4">
          <form onSubmit={handleSubmit}>
            ...
            <div className="row g-3 mb-3">
              ...
              <div className="col-md-6">
                <label htmlFor="productBrand" className="form-label">العلامة التجارية (اختياري):</label>
                <select
                  id="productBrand"
            ...
            <div className="mb-3">
              ...
              {validationErrors['additional_images.0'] && <div className="invalid-feedback">{validationErrors['additional_images.0']}</div>} {/* لأخطاء الصور الفردية */}
      ...
      {/* قائمة المنتجات الحالية - تصميم جذاب */}
                ...
                <tbody>
                  {products.map((product) => (
                    <tr key={product.product_id}>
                      ...
                      <td>
                        <button className="btn btn-sm btn-info text-white me-2" onClick={() => handleEdit(product)}>
