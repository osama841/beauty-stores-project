...
const OrderManagement = () => {
  ...
  return (
    <div className="container-fluid py-4">
      <h1 className="mb-4 fw-bold text-primary text-center text-md-start">إدارة الطلبات</h1>
      ...
      <div className="mb-4 d-flex flex-column flex-md-row justify-content-between align-items-center">
        ...
        <button className="btn btn-sm btn-outline-primary d-flex align-items-center shadow-sm" onClick={fetchOrders}>
            ...
            <>
                      ...
                      {orders.map((order) => (
                        <tr key={order.order_id}>
                          ...
                          <td><span className="text-muted small">{new Date(order.order_date).toLocaleDateString('ar-SA')}</span></td>
                           <td><span className="fw-bold">${parseFloat(order.total_amount).toFixed(2)}</span></td>
                          <td>
                          ...
                          <td>
                             <button className="btn btn-sm btn-success text-white me-2 shadow-sm" onClick={() => handleViewDetails(order.order_id)}>
                              <FaEye /> تفاصيل
                            </button>
                             <div className="dropdown d-inline-block">
                               <button className="btn btn-sm btn-secondary dropdown-toggle shadow-sm" type="button" id={`dropdownStatus${order.order_id}`} data-bs-toggle="dropdown" aria-expanded="false">
                                تغيير الحالة
                              </button>
              ...
              <div className="d-lg-none p-3">
                <div className="accordion" id="orderAccordion">
                  {orders.map((order) => (
                    <div key={order.order_id} className="accordion-item mb-2 rounded-4 shadow-sm border">
                      ...
                      <div id={`collapse${order.order_id}`} className="accordion-collapse collapse" aria-labelledby={`heading${order.order_id}`} data-bs-parent="#orderAccordion">
                        <div className="accordion-body">
                          ...
                          <div className="d-flex justify-content-end gap-2 mt-3">
                            <button className="btn btn-sm btn-success text-white shadow-sm" onClick={() => handleViewDetails(order.order_id)}>
                              <FaEye /> تفاصيل
                            </button>
                            <div className="dropdown">
                              <button className="btn btn-sm btn-secondary dropdown-toggle shadow-sm" type="button" id={`dropdownStatus${order.order_id}-mobile`} data-bs-toggle="dropdown" aria-expanded="false">
                                تغيير الحالة
                              </button>
                              <ul className="dropdown-menu" aria-labelledby={`dropdownStatus${order.order_id}-mobile`}>
                                <li><a className="dropdown-item small" href="#" onClick={() => handleUpdateStatus(order.order_id, 'pending')} style={{ color: '#343a40' }}>معلق</a></li>
                                <li><a className="dropdown-item small" href="#" onClick={() => handleUpdateStatus(order.order_id, 'processing')} style={{ color: '#343a40' }}>قيد المعالجة</a></li>
                                <li><a className="dropdown-item small" href="#" onClick={() => handleUpdateStatus(order.order_id, 'shipped')} style={{ color: '#343a40' }}>تم الشحن</a></li>
                                <li><a className="dropdown-item small" href="#" onClick={() => handleUpdateStatus(order.order_id, 'delivered')} style={{ color: '#343a40' }}>تم التسليم</a></li>
      ...
      {selectedOrder && (
        <div className={`modal fade ${showDetailModal ? 'show d-block' : ''}`} tabIndex="-1">
          {showDetailModal && <div className="modal-backdrop-custom" />}
          <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div className="modal-content rounded-4 shadow-lg">
              ...
              <div className="modal-body p-4">
                <div className="card shadow-sm mb-3 border-0 rounded bg-light">
                  <div className="card-body p-3">
                    <p className="mb-1 small text-muted"><strong>المستخدم:</strong> <span>{selectedOrder.user ? selectedOrder.user.username : 'غير معروف'}</span></p>
                    <p className="mb-1 small text-muted"><strong>البريد:</strong> <span>{selectedOrder.user ? selectedOrder.user.email : 'غير معروف'}</span></p>
                    <p className="mb-1 small text-muted"><strong>التاريخ:</strong> <span>{new Date(selectedOrder.order_date).toLocaleString('ar-SA')}</span></p>
                    <p className="mb-1 small text-muted"><strong>المبلغ الإجمالي:</strong> <span className="fw-bold">${parseFloat(selectedOrder.total_amount).toFixed(2)}</span></p>
                    <p className="mb-0 small text-muted"><strong>الحالة:</strong> <span className={`badge ${getStatusBadgeClass(selectedOrder.status)}`}>{getStatusText(selectedOrder.status)}</span></p>
                  </div>
                </div>

                <h6 className="fw-bold mb-3 mt-4 text-primary">عناصر الطلب:</h6>
                {selectedOrder.order_items && selectedOrder.order_items.length > 0 ? (
                  <ul className="list-group mb-3 rounded-lg overflow-hidden">
                    {selectedOrder.order_items.map((item) => (
                      <li key={item.order_item_id} className="list-group-item d-flex justify-content-between align-items-center py-2">
                        <div>
                          {item.product ? (
                            <>
                            ...
                            <strong className="d-block">منتج غير معروف (ID: {item.product_id})</strong>
                          )}
                        </div>
                ...
                <h6 className="fw-bold mb-3 mt-4 text-primary">عنوان الشحن:</h6>
                <div className="card shadow-sm mb-3 border-0 rounded bg-light">
                  <div className="card-body p-3">
                ...
                <h6 className="fw-bold mb-3 mt-4 text-primary">تفاصيل الدفع:</h6>
                <div className="card shadow-sm mb-3 border-0 rounded bg-light">
                  <div className="card-body p-3">
                    {selectedOrder.payment ? (
                      <>
                        <p className="mb-1 small text-muted"><strong>طريقة الدفع:</strong> <span>{selectedOrder.payment.method}</span></p>
                        <p className="mb-1 small text-muted"><strong>حالة الدفع:</strong> <span className={`badge ${selectedOrder.payment.status === 'completed' ? 'bg-success' : 'bg-danger'}`}>{selectedOrder.payment.status}</span></p>
                        <p className="mb-0 small text-muted"><strong>معرف المعاملة:</strong> <span>{selectedOrder.payment.transaction_id || 'N/A'}</span></p>
                      </>
                    ) : (
              ...
              <div className="modal-footer">
                <button type="button" className="btn btn-secondary btn-sm shadow-sm" onClick={() => setShowDetailModal(false)}>إغلاق</button>
              </div>
            </div>
          </div>
        </div>
